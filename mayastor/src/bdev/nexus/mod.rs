...
pub fn instances() -> Arc<RwLock<Vec<Arc<RwLock<Nexus>>>>> {
    nexus_module::NexusModule::get_instances()
}

/// function used to create a new nexus when parsing a config file
pub async fn nexus_instance_new(name: String, size: u64, children: Vec<String>) {
    let list = instances();
    let mut list_w = list.write().await;
    list_w.push(Nexus::new(&name, size, None, Some(&children)));
}
...
pub async fn nexus_children_to_destroying_state() {
    ...
    let nexii = instances();
    for nexus in nexii.read().await.iter() {
        for child in nexus.read().await.children.iter() {
            let mut child_w = child.write().await;
            child_w.set_state(nexus_child::ChildState::Destroying);
        }
    }
