...
async fn child_location() {
    ...
    mayastor
        .spawn(async move {
            ...
            let mut children_iter = children.values();
            assert_eq!(children_iter.next().unwrap().lock().await.is_local().unwrap(), true);
            assert_eq!(children_iter.next().unwrap().lock().await.is_local().unwrap(), false);
            assert_eq!(children_iter.next().unwrap().lock().await.is_local().unwrap(), false);
        })
        .await;
}
